...
def create_comm_struct(
    ...
    def _create(publisher: PublisherStructValue, subscription: SubscriptionStructValue):
        ...
        mocker.patch.object(communication, 'subscription_callback', callback)
        ...
        mocker.patch.object(
            communication, 'subscription_callback_name', callback.callback_name)
...
class TestCallbackRecords:
    ...
    def test_empty(
        ...
        callback = create_subscription_struct('node', 'topic', 'callback_name')
        data = Ros2DataModel()
        data.finalize()
...
class TestPublisherRecords:
    ...
    def test_single_publisher_without_tilde(
        ...
        pid = 2
        tid = 11
        data.add_rclcpp_publish(pid, tid, 1, pub_handle,
                                message_addr, message_timestamp)
        data.add_rcl_publish(pid, tid, 2, pub_handle, message_addr)
        data.add_dds_write(pid, tid, 3, message_addr)
    ...
    def test_single_publisher_with_tilde(
        ...
        data.add_tilde_subscription_init(
        ...
        data.add_rclcpp_publish(pid, tid, 2, pub_handle,
                                message_addr, message_timestamp)
        data.add_rcl_publish(pid, tid, 3, pub_handle, message_addr)
        data.add_dds_write(pid, tid, 4, message_addr)
        data.add_dds_bind_addr_to_stamp(
        ...
        df_expect = pd.DataFrame(
            ...
            columns=[
                'pid',
                'tid',
                f'{publisher_struct_mock.topic_name}/tilde_subscription',
...
class TestSubscriptionRecords:
    ...
    def test_single_records_without_tilde(
        ...
        data.add_callback_start(pid, tid, 1, callback_object, False)
        data.add_callback_end(pid, tid, 3, callback_object)
        data.finalize()
    ...
    def test_single_records_with_tilde(
        ...
        data.add_dispatch_subscription_callback(
            pid, tid, 0, callback_object, message, source_timestamp, message_timestamp)
        data.add_callback_start(pid, tid, 6, callback_object, False)
        data.add_tilde_subscribe(pid, tid, 7, tilde_sub, tilde_message_id)
        data.add_callback_end(pid, tid, 8, callback_object)
        data.finalize()
...
class TestNodeRecords:
    ...
    def test_callback_chain(
        ...
        data.add_dispatch_subscription_callback(
            pid, tid, 0, callback_object, message, source_timestamp, message_timestamp)
        data.add_callback_start(pid, tid, 1, callback_object, False)
        data.add_callback_end(pid, tid, 2, callback_object)
        data.add_callback_start(pid, tid, 3, callback_object_, False)
        data.add_rclcpp_publish(pid, tid, 4, pub_handle, 5, 6)
        data.add_callback_end(pid, tid, 5, callback_object_)
        data.finalize()
                ...
                return RecordsFactory.create_instance(
                    [
                        RecordFactory.create_instance(
                            {
                                'callback/callback_start_timestamp': 1,
                                'callback/callback_end_timestamp': 2
                            },
                        )
                    ],
                    [
                        'callback/callback_start_timestamp', 'callback/callback_end_timestamp'
                    ]
                )
            return RecordsFactory.create_instance(
                [
                    RecordFactory.create_instance(
                        {
                            'callback_/callback_start_timestamp': 3,
                            'callback_/callback_end_timestamp': 4
                        },
                    )
                ],
                [
                    'callback_/callback_start_timestamp',
                    'callback_/callback_end_timestamp'
                ]
            )
        ...
        def variable_passing_records(var_pass_args) -> RecordsInterface:
            return RecordsFactory.create_instance(
                ...
                [
                    'callback/callback_end_timestamp',
                    'callback_/callback_start_timestamp',
                ]
            )
    ...
    def test_use_latest_message(
        ...
        data.add_callback_start(pid, tid, 1, callback_object, False)
        pub_handle = 9
        data.add_rclcpp_publish(pid, tid, 2, pub_handle, 5, 6)
        data.finalize()
    ...
    def test_tilde(
        ...
        pid, tid = 5, 7
        data.add_tilde_subscription_init(pid, tid, tilde_sub, 'node', 'topic', 0)
        ...
        data.add_rcl_publish(pid, tid, 5, pub_handle, message_addr)
        data.add_dds_write(pid, tid, 6, message_addr)
...
class TestCommunicationRecords:
    ...
    def test_inter_proc(
        ...
        data.add_rcl_publish(pid, tid, 2, pub_handle, send_message)
        data.add_dds_write(pid, tid, 3, send_message)
        data.add_dds_bind_addr_to_stamp(
            pid, tid, 4, send_message, source_stamp)
        data.add_dispatch_subscription_callback(
            pid, tid, 5, callback_obj, recv_message, source_stamp, message_stamp)
        data.add_callback_start(pid, tid, 16, callback_obj, False)
        data.add_callback_end(pid, tid, 17, callback_obj)
        data.finalize()
    ...
    def test_intra_proc(
        ...
        data = Ros2DataModel()
        data.add_rclcpp_intra_publish(
            pid, tid, 1, pub_handle, message, message_stamp)
        data.add_dispatch_intra_process_subscription_callback(
            pid, tid, 5, callback_obj, message, message_stamp)
        data.add_callback_start(pid, tid, 6, callback_obj, True)
        data.add_callback_end(pid, tid, 7, callback_obj)
        data.finalize()
...
class TestTimerRecords:
    ...
    def test_timer(
        ...
        data = Ros2DataModel()
        data.add_rcl_timer_init(pid, tid, handle, timer_init_stamp, period)
        data.add_callback_start(pid, tid, 7, callback_obj, False)
        data.add_callback_end(pid, tid, 8, callback_obj)
        data.finalize()
...
class TestVarPassRecords:
    ...
    def test_var_pass(
        ...
        data.add_callback_start(pid, tid, 7, callback_obj, False)
        data.add_callback_end(pid, tid, 8, callback_obj)
        data.add_callback_start(pid, tid, 9, callback_obj_, False)
        data.add_callback_end(pid, tid, 10, callback_obj_)
        data.finalize()
        ...
        def get_callback(callback_arg):
            if callback_arg == callback_write_strct:
                return callback_write
            return callback_read

        mocker.patch.object(bridge_mock, 'get_callback', get_callback)
