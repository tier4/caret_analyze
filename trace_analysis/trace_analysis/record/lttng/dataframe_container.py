...
class DataframeContainer:
    def __init__(self, data_util: Ros2DataModelUtil):
        ...
        data_util.data.nodes = self.merge_namespace_and_name_coumn(data_util.data.nodes)
    ...
    def get_publisher_handles(self, topic_name, node_name=None):
        ...
        if node_name is None:
            ...
            df_target = df[(df["name"] == node_name) & (df["topic_name"] == topic_name)]
    ...
    def get_subscription_info(self) -> pd.DataFrame:
        ...
        sub_df = pd.merge(
            sub_df,
            self._data_util.data.subscription_objects.drop(["timestamp"], axis=1),
            left_index=True,
        ...
        sub_df = pd.merge(
            sub_df,
            self._data_util.data.nodes.drop(["timestamp", "tid", "rmw_handle"], axis=1),
            left_on="node_handle",
            right_index=True,
        )
