...
class Ros2Handler():
    ...
    def __init__(
        ...
        self.context_handle_remapper = IDRemapper()
        self.node_handle_remapper = IDRemapper()
        self.publisher_handle_remapper = IDRemapper()
        self.rmw_publisher_handle_remapper = IDRemapper()
        self.subscription_handle_remapper = IDRemapper()
        self.rmw_subscription_handle_remapper = IDRemapper()
        self.subscription_remapper = IDRemapper()
        self.service_handle_remapper = IDRemapper()
        self.rmw_service_handle_remapper = IDRemapper()
        self.client_handle_remapper = IDRemapper()
        self.rmw_client_handle_remapper = IDRemapper()
        self.timer_handle_remapper = IDRemapper()
        self.state_machine_remapper = IDRemapper()
        self.executor_addr_remapper = IDRemapper()
        self.entities_collector_addr_remapper = IDRemapper()
        self.callback_group_addr_remapper = IDRemapper()
        self.callback_remapper = IDRemapper()
    ...
    def _handle_rcl_init(
        ...
        # 〇 context_handle
        context_handle = self.context_handle_remapper.register_and_get_object_id(
    ...
    def _handle_rcl_node_init(
        ...
        # 〇 context_handle
        handle = self.node_handle_remapper.register_and_get_object_id(handle, event)
    ...
    def _handle_rcl_publisher_init(
        ...
        handle = self.publisher_handle_remapper.register_and_get_object_id(handle, event)
        node_handle = self.node_handle_remapper.get_object_id(node_handle, event)
        rmw_handle = self.rmw_publisher_handle_remapper.register_and_get_object_id(
            rmw_handle, event)
    ...
    def _handle_rcl_subscription_init(
        ...
        handle = self.subscription_handle_remapper.register_and_get_object_id(handle, event)
        node_handle = self.node_handle_remapper.get_object_id(node_handle, event)
        rmw_handle = self.rmw_subscription_handle_remapper.register_and_get_object_id(
            rmw_handle, event)
    ...
    def _handle_rclcpp_subscription_init(
        ...
        # △ subscription_handle
        # 〇 subscription
        handle = self.subscription_handle_remapper.get_object_id(handle, event)
        subscription_pointer = self.subscription_remapper.register_and_get_object_id(
    ...
    def _handle_rclcpp_subscription_callback_added(
        ...
        # △ subscription
        # 〇 callback
        subscription_pointer = self.subscription_remapper.get_object_id(
            subscription_pointer, event)
        callback_object = self.callback_remapper.register_and_get_object_id(callback_object, event)
    ...
    def _handle_rcl_service_init(
        ...
        handle = self.service_handle_remapper.register_and_get_object_id(handle, event)
        node_handle = self.node_handle_remapper.get_object_id(node_handle, event)
        rmw_handle = self.rmw_service_handle_remapper.register_and_get_object_id(rmw_handle, event)
    ...
    def _handle_rclcpp_service_callback_added(
        ...
        # △ service_handle
        # 〇 callback
        handle = self.service_handle_remapper.get_object_id(handle, event)
        callback_object = self.callback_remapper.register_and_get_object_id(callback_object, event)
    ...
    def _handle_rcl_client_init(
        ...
        # △ node_handle
        # 〇 rmw_client_handle
        handle = self.client_handle_remapper.register_and_get_object_id(handle, event)
        node_handle = self.node_handle_remapper.get_object_id(node_handle, event)
        rmw_handle = self.rmw_client_handle_remapper.register_and_get_object_id(rmw_handle, event)
    ...
    def _handle_rcl_timer_init(
        ...
        # 〇 timer_handle
        handle = self.timer_handle_remapper.register_and_get_object_id(handle, event)
    ...
    def _handle_rclcpp_timer_callback_added(
        ...
        if callback_object == 139926729236880:
            myk = 1
            print(myk)
        #    callback_object = self.timer_handle_remapper.register_and_get_object_id(
        #       callback_object, event)
        #    print("_handle_rclcpp_timer_callback_added()
        #       timer_handle_remapper.register_and_get_object_id : ",callback_object)
            # from caret_analyze.infra.lttng.id_remapper import IDRemapper
            # id_remap = IDRemapper()
        ...
        # △ timer_handle
        # 〇 callback
        handle = self.timer_handle_remapper.get_object_id(handle, event)
        callback_object = self.callback_remapper.register_and_get_object_id(callback_object, event)
        self.data.add_callback_object(handle, timestamp, callback_object)
    ...
    def _handle_rclcpp_callback_register(
        ...
        #    callback_object = self.timer_handle_remapper.get_object_id(callback_object, event)
        #    print("after : ",callback_object)
        #    print("_handle_rclcpp_callback_register()
        #       timer_handle_remapper.get_object_id : ",callback_object)
        #    print(len(self._data._callback_symbols._data['callback_object']))
        #    print(event)
        ############################### MYK テスト完了後あとで消す <--

        # △ callback
        callback_object = self.callback_remapper.get_object_id(callback_object, event)
    ...
    def _handle_callback_start(
        ...
        # △ callback
        callback = self.callback_remapper.get_object_id(callback, event)
    ...
    def _handle_rcl_lifecycle_state_machine_init(
        ...
        # 〇 state_machine
        state_machine = self.state_machine_remapper.register_and_get_object_id(
            state_machine, event)
    ...
    def _handle_rcl_lifecycle_transition(
        ...
        state_machine = self.state_machine_remapper.get_object_id(state_machine, event)
    ...
    def _handle_rclcpp_publish(
        ...
        # △ publisher_handle
        # 【TODO】message
        publisher_handle = self.publisher_handle_remapper.get_object_id(publisher_handle, event)
    ...
    def _handle_rmw_take(
        ...
        # 【TODO】message
        rmw_subscription_handle = self.rmw_subscription_handle_remapper.get_object_id(
    ...
    def _handle_construct_executor(
        ...
        # 〇 executor_addr
        executor_addr = self.executor_addr_remapper.register_and_get_object_id(
            executor_addr, event)
    ...
    def _handle_construct_static_executor(
        ...
        executor_addr = self.executor_addr_remapper.register_and_get_object_id(
            executor_addr, event)
        collector_addr = self.entities_collector_addr_remapper.register_and_get_object_id(
    ...
    def _handle_add_callback_group(
        ...
        executor_addr = self.executor_addr_remapper.get_object_id(executor_addr, event)
        callback_group_addr = self.callback_group_addr_remapper.register_and_get_object_id(
    ...
    def _handle_add_callback_group_static_executor(
        ...
        # 〇 callback_group_addr
        collector_addr = self.entities_collector_addr_remapper.get_object_id(collector_addr, event)
        callback_group_addr = self.callback_group_addr_remapper.register_and_get_object_id(
            callback_group_addr, event)
    ...
    def _handle_callback_group_add_timer(
        ...
        # △ callback_group_addr
        # △ timer_handle
        callback_group_addr = self.callback_group_addr_remapper.get_object_id(
            callback_group_addr, event)
        timer_handle = self.timer_handle_remapper.get_object_id(timer_handle, event)
    ...
    def _handle_callback_group_add_subscription(
        ...
        callback_group_addr = self.callback_group_addr_remapper.get_object_id(
            callback_group_addr, event)
        subscription_handle = self.subscription_handle_remapper.get_object_id(
    ...
    def _handle_callback_group_add_service(
        ...
        callback_group_addr = self.callback_group_addr_remapper.get_object_id(
            callback_group_addr, event)
        service_handle = self.service_handle_remapper.get_object_id(service_handle, event)
    ...
    def _handle_callback_group_add_client(
        ...
        callback_group_addr = self.callback_group_addr_remapper.get_object_id(
            callback_group_addr, event)
        client_handle = self.client_handle_remapper.get_object_id(client_handle, event)
