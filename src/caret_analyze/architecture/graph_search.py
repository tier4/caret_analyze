...
class CallbackPathSearcher:
    ...
    def search(
        ...
        for graph_path in graph_paths:
            ...
            paths += self._to_paths(graph_path, start_callback, end_callback, subscription, publisher)
    ...
    def _to_paths(
        ...
        end_callbck_publishers: Optional[List[PublisherStruct]]
    ) -> List[NodePathStruct]:

        if subscription is None and len(end_callbck_publishers or []) == 0:
            return [] # subかpubのどちらかすら無い場合はパスとして算出しない。

        if end_callbck_publishers is None or len(end_callbck_publishers) == 0:
        ...
        return [
            self._to_path(callback_graph_path,
                          ...
                          publisher)
            for publisher
            in end_callbck_publishers
            # publisher側にはconstruction_orderを入れないほうが良い。
        ]
...
class NodePathSearcher:
    ...
    def __init__(
        ...
        for comm in communications:
            ...
            # @を区切り文字として、各情報を追加する。
            edge_label = f'{comm.topic_name}@{comm.subscription_construction_order}@{comm.publisher_construction_order}'
            self._graph.add_edge(
    ...
    def _create_head_dummy_node_path(
        ...
        publisher = NodePathSearcher._get_publisher(nodes, node_name, topic_name, construction_order)
    ...
    def _create_tail_dummy_node_path(
        ...
        node_name: str,
        topic_name: str,
        subscription_construct_order: int
    ) -> NodePathStruct:
        sub = NodePathSearcher._get_subscription(nodes, node_name, topic_name, subscription_construct_order)
    ...
    def _to_path(
        ...
        # add tail node path
        tail_node_path = self._create_tail_dummy_node_path(self._nodes, tail_edge.node_name_to, topic_name, sub_const)
